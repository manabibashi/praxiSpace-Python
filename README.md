...
コードは2つの方法で見られます。どちらも `main` ブランチの内容が最新版です。

- **GitHub の画面で読む**: 下の目次のリンクをクリックすると、そのままコードが読めます。
- **自分のパソコンにダウンロードして動かす**: 次のコマンドでリポジトリを手元にコピーできます。

  ```
  git clone https://github.com/manabibashi/praxiSpace-Python.git
  ```

  コピーしたら、たとえば `python lecture/1_print.py` のように実行できます。
...
このリポジトリは `main` ブランチだけで運用しています。修正は `main` に向けたプルリクエストを作ってください。

プルリクエストを作ると、すべての Python ファイルに文法の誤りがないかを自動でチェックします。チェックが通ったらマージできます。
